...
import { parseNetworkFlag, resolveNetwork, setActiveNetwork } from "./network";

function run(cmd: string, args: string[]): void {
  const r = spawnSync(cmd, args, { shell: false, stdio: "inherit" });
  if (r.status !== 0) {
...
function main(): void {
  ...
  const deployArgs =
    network === "undeployed" ? [] : ["--", "--network", network];
